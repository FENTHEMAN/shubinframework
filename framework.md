...
The design system lives in **two places at once** — Figma (component library, variables for tokens) and code (`shared/design-system/` with components, Tailwind config or CSS variables with the same tokens) — and the two sync manually but in agreement: new component in Figma first, then in code; design changes that aren't yet in code → designer opens a PR or task; once a month, a 30-minute meeting verifying alignment. The detailed mechanics — file structure, Tokens Studio pipeline, handoff workflow — vary by team; fix them in `patterns/design-system.md` for your team. When you need to write the starter recipe down, put it in `guides/design-process.md`.
